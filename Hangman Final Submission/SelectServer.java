              ...
              // System.out.print("while looping \n");
                ...
                while (readyItor.hasNext())
                {
                    ...
                    // Read the channel
                            ...
                            if (bytesRecv <= 0) {
                                ...
                                String checkHost = "";
                                  for (int i = 0; i < users.size();i++) {
                                      if (users.get(i).getChannel() == cchannel) {
                                          checkHost = users.get(i).getUsername();
                                          users.remove(i);
                                          channels.remove(i);
                                          break;
                                          }
                                          ...
                                          for (int j = 0; j < userOrder.size();j++) {
                                            if (userOrder.get(j).getChannel() == cchannel ) {
                                              userOrder.remove(j);
                                              break;
                                            }
                            ...
                            // If they are, add them to the list of users and add their channel to the list of channels
                                ...
                                for (User user : users ) {
                                    ...
                                    if (check.equals(username)) {
                                        alreadyLogged = true;
                                        outLine = "You are already logged in. Please try a different command or /logout first.\n";
                                        break;
                                    }
                                }
                                ...
                                if (!alreadyLogged) {
                                    ...
                                    "^    (you can't set a word until at least one player joins the game)" +
                                    "^2. /gl <letter> to guess a letter (if you are next player)" +
                                    ...
                                    if (gameRunning) {
                                      ...
                                      outLine = outLine + gameBoard + "^";
                                      String currProgress = new String(newGame.getTemplate());
                                      outLine =  outLine + "Current word to guess: " + currProgress + "^";
                                      outLine =  outLine + "Current player is " + userOrder.get(0).getUsername() + ".\n";
                                      String guesses = new String(newGame.getGuesses());
                                      outLine =outLine +  "Current guesses: " + guesses + "\n";
                                    }
                                }
                                else {
                        ...
                        if (inLine.startsWith("/setWord") && name.equals(host) && users.size() >= 2) {
                          ...
                          else if (inLine.startsWith("/setWord") && !name.equals(host)) {
                        ...
                        // Of the format: /gl <letter>
                        else if (inLine.startsWith("/gl") && !host.equals(name)) {
                            ...
                            if (newGame.check(newGame.getGuesses(), l)) {
                                ...
                                if(newGame.guessLetter(l)) {
                                    outLine = "New letter successfully guessed: "+ inLine.split(" ")[1] +  "^";
                                    String gameBoard = new String(newGame.gameBoard());
                                    outLine = outLine + gameBoard + "^";
                                    String currProgress = new String(newGame.getTemplate());
                                    outLine =  outLine + "Current word to guess: " + currProgress + "^";
                                    ...
                                    if (!(new String(newGame.getTemplate()).contains("*"))) {
                                        ...
                                        host = userOrder.get(userOrder.size()).getUsername(); // With the previous host at the back of the list, this now selects whoever is at the front
                                        userOrder.remove(userOrder.size());

                                        // Choose the next person to be the host
                                        for (int i = 0; i< users.size();i++) {
                                          if (users.get(i).getUsername().equals(host)){
                                            userOrder.add(users.get(i));
                                          }
                                        }
                                      ...
                                      if (userOrder.size() == 1) {
                                      outLine = "Current player is " + userOrder.get(0).getUsername() + ".\n";
                                    } else if (userOrder.size() >= 2) {
                                      outLine = "Current player is " + userOrder.get(0).getUsername() + ".\n";
                                      outLine = outLine + "Next player is " + userOrder.get(1).getUsername() + "\n";
                                      }
                                      for (User u : users) {
                                          outBuffer = encoder.encode(CharBuffer.wrap(outLine));
                                          u.getChannel().write(outBuffer);
                                ...
                                // If the letter guessed is an incorrect guess then the turn passes
                                    ...
                                    outLine = "New letter incorrectly guessed:"+ inLine.split(" ")[1] +  "^";
                                    String gameBoard = new String(newGame.gameBoard());
                                    outLine = outLine + gameBoard + "^";
                                    String currProgress = new String(newGame.getTemplate());
                                    outLine =  outLine + "Current word to guess: " + currProgress + "^";
                                    ...
                                    if(newGame.getIncorrectGuesses() >= 6) {
                                        ...
                                        userOrder.remove(0);
                                        // Send the game lost message to every player
                                        outLine = outLine + host+ " is now host. Waiting on them to set the word.\n ";
                                        for (User u : users) {
                                            outBuffer = encoder.encode(CharBuffer.wrap(outLine));
                                            u.getChannel().write(outBuffer);
                                            outBuffer.clear();
                                    ...
                                    for (int j = 0; j < userOrder.size();j++) {
                                      ...
                                      if (host.equals(checkHost)) {
                                          if (users.size() >= 1) {
                                              ...
                                              if (users.size() == 1) {
                                                newGame.resetGame();
                                                outLine = outLine + "The game has reset due to everyone leaving. ^" +
                                                                                  "When someone else joins you may set the word.\n";
                                              }
                                              userOrder.remove(0);
                                              users.get(0).getChannel().write(outBuffer);
                                              outBuffer.clear();
